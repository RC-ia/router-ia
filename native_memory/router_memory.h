...
// Backward-compatible raw memory API.
...
int router_mem_stage_host(
    RouterMemoryManager* m,
    uint32_t ram_slot,
    const void* src,
    uint64_t bytes);

int router_mem_h2d_async(
    RouterMemoryManager* m,
    uint32_t ram_slot,
    uint32_t vram_slot,
    uint64_t bytes);

int router_mem_d2h_async(
    RouterMemoryManager* m,
    uint32_t vram_slot,
    uint32_t ram_slot,
    uint64_t bytes);
...
// Logical block memory-manager API.
// A logical block owns one RAM backing slot and optionally one VRAM slot.
int router_mm_register_block(
    RouterMemoryManager* m,
    uint32_t block_id,
    const void* src,
    uint64_t bytes);

int router_mm_unregister_block(
    RouterMemoryManager* m,
    uint32_t block_id);

int router_mm_is_registered(
    RouterMemoryManager* m,
    uint32_t block_id);

int router_mm_is_resident(
    RouterMemoryManager* m,
    uint32_t block_id);

// Make a logical block resident in VRAM.
// On miss, the manager evicts an unpinned LRU block when necessary.
// Returns the physical VRAM slot through out_vram_slot.
int router_mm_acquire(
    RouterMemoryManager* m,
    uint32_t block_id,
    uint64_t bytes,
    uint32_t* out_vram_slot);

// Mark a block as recently used without changing pin state.
int router_mm_touch(
    RouterMemoryManager* m,
    uint32_t block_id);

// Prevent eviction while a caller is using the block.
int router_mm_pin(
    RouterMemoryManager* m,
    uint32_t block_id);
...
// Explicitly evict a block. Fails when pinned.
int router_mm_evict(
    RouterMemoryManager* m,
    uint32_t block_id);

// Wait for all outstanding transfers and return manager statistics.
RouterMemoryStats router_mm_stats(RouterMemoryManager* m);

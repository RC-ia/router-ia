...
class RoutedExpertCache:
    """Fixed-size per-layer LRU cache for complete routed experts."""
    ...
    def __init__(self, budget_bytes: int, layers: int = MODEL_LAYERS) -> None:
        ...
        # Five complete experts per layer was empirically much better than the
        # adaptive 2-minimum + shared-LRU policy. It prevents cross-layer
        # thrashing while keeping the full ~1.2 GiB expert budget bounded.
        max_slots_by_budget = (
        ...
        self.slots_per_layer = (
            min(SLOTS_PER_LAYER, max_slots_by_budget // self.layers)
            if max_slots_by_budget
            else 0
        )
        self.slots_per_layer = max(self.slots_per_layer, 0)
        self.layer_budgets = {
    ...
    def put(self, layer: int, expert_id: int, entry):
        ...
        with self.lock:
            ...
            # Evict only from the same layer. Other layers retain their
            # working set throughout the sequential 40-layer decode sweep.
            while bank and (
    ...
    def snapshot(self) -> dict[str, int | float]:
        with self.lock:
            ...
            return {
                ...
                "fixed_slots_per_layer": SLOTS_PER_LAYER,
            }

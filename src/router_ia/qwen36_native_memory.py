...
def _candidate_paths() -> list[Path]:
    ...
    native = root / "native_memory" / "build"
    for name in (
        ...
        candidates.append(native / "Release" / name)
        candidates.append(native / "Debug" / name)
    return candidates
...
class NativeMemory:
    def __init__(
        ...
        self._configure_abi()
        self.handle = self.lib.router_mem_create(
            int(vram_slot_bytes), int(vram_slots), int(ram_slot_bytes), int(ram_slots)
        )
        if not self.handle:
            raise NativeMemoryError("router_mem_create failed; check CUDA availability/memory")
    ...
    def _configure_abi(self) -> None:
        lib = self.lib
        lib.router_mem_create.argtypes = [ctypes.c_uint64, ctypes.c_uint32, ctypes.c_uint64, ctypes.c_uint32]
        ...
        lib.router_mem_stage_host.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.c_void_p, ctypes.c_uint64]
        lib.router_mem_stage_host.restype = ctypes.c_int
        lib.router_mem_h2d_async.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint32, ctypes.c_uint64]
        lib.router_mem_h2d_async.restype = ctypes.c_int
        lib.router_mem_d2h_async.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint32, ctypes.c_uint64]
        ...
        lib.router_mem_zero_vram.argtypes = [ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint64]
    ...
    def stage_host(self, ram_slot: int, src: Any, bytes: int) -> None:
        ...
        if not self.lib.router_mem_stage_host(self.handle, int(ram_slot), src_ptr, int(bytes)):
            raise NativeMemoryError("router_mem_stage_host failed")

    def h2d_async(self, ram_slot: int, vram_slot: int, bytes: int) -> None:
        if not self.lib.router_mem_h2d_async(self.handle, int(ram_slot), int(vram_slot), int(bytes)):
            raise NativeMemoryError("router_mem_h2d_async failed")

    def d2h_async(self, vram_slot: int, ram_slot: int, bytes: int) -> None:
        if not self.lib.router_mem_d2h_async(self.handle, int(vram_slot), int(ram_slot), int(bytes)):
    ...
    def zero_vram(self, vram_slot: int, bytes: int | None = None) -> None:
        ...
        if not self.lib.router_mem_zero_vram(self.handle, int(vram_slot), count):

...
"""Compatibility-fixed FP8 expert cache.

Indexes Qwen3.6 FP8 Safetensors experts using the file header instead of
safe_open.get_dtype(), which is absent in some safetensors versions.
"""
...
class FP8ExpertIndex:
    ...
    def _discover_shards(self) -> list[Path]:
        ...
        if index_path.is_file():
            ...
            return [self.root / n for n in names if (self.root / n).is_file()]
    ...
    def _build(self) -> None:
        ...
        for shard in self._discover_shards():
            ...
            for name, meta in read_header(shard).items():
                ...
                if m:
                    key = (int(m.group("layer")), int(m.group("expert")))
                    weights.setdefault(key, {})[m.group("kind")] = ref
                    continue
                m = _SCALE_RE.match(name)
                if m:
                    key = (int(m.group("layer")), int(m.group("expert")))
                    scales.setdefault(key, {})[m.group("kind")] = ref
        ...
        self.layer_count = max(k[0] for k in self.records) + 1
        self.expert_count = max(k[1] for k in self.records) + 1

    def get(self, layer: int, expert: int) -> ExpertRecord:
        key = (int(layer), int(expert))
        if key not in self.records:
            raise KeyError(f"Unknown FP8 expert: layer={layer}, expert={expert}")
...
class FP8ExpertCache:
    ...
    def _load_blob(self, key: ExpertKey) -> ExpertBlob:
        ...
        weights = {k: self._load_tensor(v) for k, v in rec.weights.items()}
        scales = {k: self._load_tensor(v) for k, v in rec.scales.items()}
    ...
    def get(self, layer: int, expert: int, *, tier: str = "vram") -> ExpertBlob:
        return self.get_ram(layer, expert) if tier == "ram" else self.promote_to_vram(layer, expert)
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            "ram_used_bytes": self.ram_used, "ram_limit_bytes": self.ram_limit,
            "vram_used_bytes": self.vram_used, "vram_limit_bytes": self.vram_limit,
            "ram_entries": len(self.ram), "vram_entries": len(self.vram),
            "ram_keys_mru": list(reversed(self.ram)), "vram_keys_mru": list(reversed(self.vram)),
            "stats": {
                "ram_hits": self.stats.ram_hits, "ram_misses": self.stats.ram_misses,
                "vram_hits": self.stats.vram_hits, "vram_misses": self.stats.vram_misses,
                "loads_from_disk": self.stats.loads_from_disk, "ram_to_vram": self.stats.ram_to_vram,
                "vram_evictions": self.stats.vram_evictions, "ram_evictions": self.stats.ram_evictions,
                "ram_hit_rate": self.stats.ram_hit_rate, "vram_hit_rate": self.stats.vram_hit_rate,
            },
        }
...
def main() -> None:
    ...
    cache = FP8ExpertCache(args.root, ram_limit_bytes=int(args.ram_gb*1024**3),
    ...
    if args.sequence:
        ...
        print(json.dumps({"expert": [args.layer,args.expert], "size_bytes": blob.size_bytes,
                          "weight_shapes": {k:list(v.shape) for k,v in blob.weights.items()},
                          "scale_shapes": {k:list(v.shape) for k,v in blob.scales.items()}}, indent=2))

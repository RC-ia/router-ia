...
The test is specifically designed for large FP8 MoE checkpoints: it does not
instantiate Qwen3_5MoeForCausalLM, so it avoids the global MoE expert-conversion
step that can exhaust multi-GPU VRAM before forward() starts.
"""
...
def _reference_step(
    ...
    """Independent single-token PyTorch reference of the GDN equations."""
...
def main() -> None:
    ...
    ref_conv = torch.zeros(
        1, ours.LINEAR_CONV_DIM, ours.LINEAR_CONV_STATE,
        device=args.device, dtype=conv_dtype,
    )
    ...
    try:
        for index in range(args.tokens):
            ...
            ref_out, ref_state, ref_conv = _reference_step(
            ...
            step_ok = (
                output_diff[0] <= args.tolerance
                and state_diff[0] <= args.tolerance
                and conv_diff[0] <= args.tolerance
            )
            all_ok &= step_ok

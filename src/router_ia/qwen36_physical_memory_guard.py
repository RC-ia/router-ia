...
def _guarded_ram_put(self, name, tensor):
    ...
    try:
        ...
        # If physical RAM is already tight, allow the normal eviction mechanism
        # to make room rather than letting Python/Torch rely on pagefile-backed
        # committed memory for the cache.
        return _ORIGINAL_RAM_PUT(self, name, tensor)
    finally:
...
def _memory_snapshot() -> tuple[int, int, int, int]:
    available = _physical_available_bytes()
    generic = _generic_ram_bytes()
    q4 = _q4_ram_bytes()
    managed = generic + q4
    return available, generic, q4, managed


def _print_memory_status(root: Path, label: str) -> None:
    available, generic, q4, managed = _memory_snapshot()
    if available <= 0:

...
def main() -> None:
    ...
    for layer in range(args.start_layer, args.end_layer + 1):
        ...
        residual = linear_attention_step(root, layer, x, device) if kind == "linear_attention" else full_attention_step(root, layer, x, device)
        x, expert_ids, weights, shared_gate, moe_input_norm = moe_step(root, layer, residual, args.top_k, device)
        torch.cuda.synchronize()

...
def load_one_tensor_cpu(
    ...
    with safe_open(str(shard), framework="pt", device="cpu") as handle:
        logger.log(
            f"OPEN OK | shard={shard.name} | open_ms={(perf_counter()-t0)*1000:.2f} | "
            f"keys={len(handle.keys())} | {memory_line()}"
        )
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        logger.log(f"Torch={torch.__version__} | CUDA available={torch.cuda.is_available()} | stage={args.stage}")
        if torch.cuda.is_available():
            logger.log(
                f"CUDA device={torch.cuda.get_device_name(0)} | capability={torch.cuda.get_device_capability(0)}"
            )
        ...
        logger.log(
            f"INDEX READ OK | tensors_in_index={len(weight_map)} | elapsed_ms={(perf_counter()-t0)*1000:.2f}"
        )
        ...
        logger.log(
            f"LOAD PLAN | selected_tensors={len(selected)} | stage={args.stage} | limit={args.limit}"
        )
        ...
        for seq, (name, shard_name) in enumerate(selected, 1):
            ...
            layer = layer_of(name)
            suffix = (
                name.split(f"model.language_model.layers.{layer}.", 1)[-1]
                if layer != "global"
                else name
            )
            expected = expected_suffix.get(suffix)
            logger.log(
                f"STEP {seq}/{len(selected)} BEGIN | layer={layer} | tensor={name} | "
                f"shard={shard.name} | expected_shape={expected}"
            )
            ...
            if shard_name not in shard_cache:
                ...
                logger.log(
                    f"SHARD DISCOVERY OK | shard={shard.name} | tensor_count={len(shard_cache[shard_name])}"
                )
            ...
            if args.stage in {"cuda", "dequant", "full"}:
                ...
                logger.log(
                    f"CUDA COPY ABOUT_TO_START | tensor={name} | bytes={tensor_nbytes(tensor)} | {memory_line()}"
                )
                t0 = perf_counter()
                gpu_tensor = tensor.to(device="cuda", non_blocking=False)
                logger.log(
                    f"CUDA COPY ENQUEUED | tensor={name} | elapsed_ms={(perf_counter()-t0)*1000:.2f} | {memory_line()}"
                )
                logger.log(f"CUDA SYNC ABOUT_TO_START | tensor={name} | {memory_line()}")
                t0 = perf_counter()
                torch.cuda.synchronize()
                logger.log(
                    f"CUDA SYNC OK | tensor={name} | elapsed_ms={(perf_counter()-t0)*1000:.2f} | "
                    f"gpu_shape={tuple(gpu_tensor.shape)} | gpu_dtype={gpu_tensor.dtype} | {memory_line()}"
                )
            else:
                gpu_tensor = None
            ...
            if args.stage in {"dequant", "full"} and tensor.ndim == 2 and tensor.dtype == torch.float8_e4m3fn:
                ...
                logger.log(
                    f"FP8 SCALE LOAD ABOUT_TO_START | tensor={scale_name} | shard={scale_shard.name}"
                )
                scale = load_one_tensor_cpu(logger, scale_shard, scale_name, None)
                if args.stage == "full":
                    logger.log(
                        f"FP8 DEQUANT ABOUT_TO_START | weight={name} | scale={scale_name} | "
                        f"device=cuda | {memory_line()}"
                    )
                    ...
                    logger.log(
                        f"FP8 DEQUANT KERNEL ENQUEUED | tensor={name} | "
                        f"elapsed_ms={(perf_counter()-t0)*1000:.2f} | out_shape={tuple(out.shape)} | {memory_line()}"
                    )
                    logger.log(f"FP8 DEQUANT SYNC ABOUT_TO_START | tensor={name}")
                    torch.cuda.synchronize()
                    logger.log(
                        f"FP8 DEQUANT OK | tensor={name} | out_dtype={out.dtype} | {memory_line()}"
                    )
                    del out, scale_gpu
                del scale
            ...
            completed += 1
            logger.log(
                f"STEP {seq}/{len(selected)} COMPLETE | tensor={name} | completed={completed} | {memory_line()}"
            )

...
def _expert_projection_triplet(root: Path, layer_prefix: str, expert_id: int, device: str):
...
def _warm_expert_raw_cache(root: Path, layer_prefix: str, expert_ids: list[int]) -> None:
    ...
    from .qwen36_chat_batch_fused import _EXPERT_CACHES
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(expert_cache.prefetch_expert_raw, store, layer_prefix, int(expert_id)) for expert_id in expert_ids]
        for future in futures:
            future.result()


def batched_moe_step(root: Path, layer: int, residual: torch.Tensor, top_k: int, device: str):
...
def run_forward_token(root: Path, token_id: int, final_norm: torch.Tensor, lm_head: torch.Tensor, final_norm_name: str, lm_head_name: str, device: str, advance_state: bool = True):
    start = perf_counter()
